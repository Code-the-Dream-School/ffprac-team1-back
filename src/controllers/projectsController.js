...
const displaySearchProjects = asyncWrapper(async (req, res) => {
  ...
  if (search) {
    ...
    const results = await Project.find(query, projection).sort(sort).skip(skip).limit(limit);
    ...
    const detailedResults = await Promise.all(
      results.map(project => {
        ...
        let searchableText =
          `${projectObj.title} ${projectObj.description} ` +
          (projectObj.technologies ? `${Object.values(projectObj.technologies).flat().join(" ")}` : "") +
          ` ${projectObj.rolesNeeded.join(" ")}`;

        searchableText = searchableText.toLowerCase();

        projectObj.missingWords = searchWords.filter(word => !searchableText.includes(word.toLowerCase()));
...
const rejectApplicant = asyncWrapper(async (req, res, next) => {
  ...
  if (
    ...
    !project.applicants.some(
      applicant => applicant.user.toString() === applicationId && applicant.user.toString() === userId
    )
  ) {
...
const removeParticipant = asyncWrapper(async (req, res, next) => {
  ...
  if (project.createdBy.toString() !== userId) {
    return res
      ...
      .json({ message: "You do not have permission to remove participants from this project." });
  }
  ...
  user.participatingProjects = user.participatingProjects.filter(
    participation => participation.project.toString() !== projectId
  );
  await user.save();
