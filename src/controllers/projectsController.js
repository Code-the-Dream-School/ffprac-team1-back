...
const displaySearchProjects = asyncWrapper(async (req, res) => {
  ...
  if (search) {
    ...
    const regexQueries = searchWords.map((word) => {
      ...
      return {
        $or: [
          //$or condition: if it matches any of the conditions listed within the array
      ...
      if (allMatches.length < limit) {
        ...
        const findMissingWords = (sourceText, searchWords) => {
          return searchWords.filter(
            (word) => !sourceText.toLowerCase().includes(word.toLowerCase())
          );
        };
    ...
    const data = projects.map((project) => {
      ...
      const isCreator =
        req.user && req.user.userId === project.createdBy.toString();
      projectObj.applicants = isCreator ? project.applicants : undefined;

      projectObj.participants =
