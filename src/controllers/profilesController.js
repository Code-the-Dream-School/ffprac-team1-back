...
const getUserProfile = async (req, res) => {
  ...
  try {
    const profile = await User.findById(idToSearch).select(
    ...
    res
      .status(500)
      .json({ message: 'The profile is unavailable. Try again later please' });
  }
};
...
const updateUserProfile = async (req, res) => {
  ...
  try {
    if (
      ...
      'email' in updateData ||
      'passwordResetToken' in updateData
    ) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        message:
          'Direct updates to email, password, or password reset token are not allowed through this endpoint.',
      });
    }
